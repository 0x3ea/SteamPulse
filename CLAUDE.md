...
Phase 1 进行中：**gin 骨架已搭好并跑通**（walking skeleton，`/healthz` 返回 200）。完整开发计划见 `.claude/plan.md`——动工前必读。
module path 已定为 `github.com/0x3ea/SteamPulse`（`go 1.25.0` + gin v1.12.0）；项目名 SteamPulse。
...
- ⚠️ 不要在 Phase 1 就动手做 Phase 2 的监控/轮询。
...
- 本地验证：`./bin/server`，另开终端 `curl localhost:8080/healthz` → 应返回 `{"status":"ok"}`
...
- ⚠️ **`os.Getenv` 不自动加载 `.env`**：直接 `go run` 读不到 key。运行前要么 `export STEAM_API_KEY=...`，要么用 `godotenv` 自动加载（做 `internal/steam` client 时会处理）。
- `/healthz` 是**故意零依赖**的存活探针：不查 DB、不调 Steam API；200 = 进程在跑、HTTP 层通。Docker / 负载均衡 / 本地 curl 都靠它。新功能路由在 `internal/adapter/http/router.go` 的 `NewRouter()` 里加。
